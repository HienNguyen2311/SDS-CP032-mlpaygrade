...
long_to_short_exp = {'Entry': 'EN', 'Mid': 'MI', 'Senior': 'SE', 'Executive': 'EX'}
long_to_short_emp = {'Full-Time': 'FT', 'Part-Time': 'PT', 'Contract': 'CT', 'Freelance': 'FL'}
...
def preprocess_salary(experience_level, employment_type, job_title, salary_currency,
    ...
    user_input_dict = {
        'work_year': ['2024'],
        'experience_level0': [experience_level],
        'employment_type0': [employment_type],
        'job_title': [job_title],
        'salary_currency': [salary_currency],
        'employee_residence0': [employee_residence],
        'remote_ratio': [remote_ratio],
        'company_location0': [company_location],
        'company_size': [company_size]
    }
    user_input = pd.DataFrame(data=user_input_dict)
    user_input2 = user_input.copy()
    user_input2['experience_level'] = user_input['experience_level0'].map(long_to_short_exp)
    user_input2['employment_type'] = user_input['employment_type0'].map(long_to_short_emp)
    user_input2['employee_residence'] = user_input['employee_residence0'].map(country_name_to_iso2)
    user_input2['company_location'] = user_input['company_location0'].map(country_name_to_iso2)
    user_input2['job_type'] = user_input['job_title'].apply(job_type_label)
    user_input2['employee_continent'] = user_input2['employee_residence'].map(country_to_continent)
    user_input2['company_continent'] = user_input2['company_location'].map(country_to_continent)
